...
def build_prompt(tokenizer, messages):
    # Keep system + user and let model generate assistant answer.
    msgs = []
    for m in messages:
        role = m.get("role")
...
def generate_batch(model, tokenizer, prompt: str, max_new_tokens: int):
...
def run_eval(model, tokenizer, dataset, out_path: str, max_new_tokens: int, tag: str):
    ...
    with open(out_path, "w", encoding="utf-8") as fw:
        for i, item in enumerate(dataset, start=1):
            ...
            pred = generate_batch(model, tokenizer, prompt, max_new_tokens=max_new_tokens)
...
def main():
    ...
    # Release before loading adapter model
    del base_model
    ...
    run_eval(
        ...
        tokenizer2,
        internal,
        os.path.join(args.out_dir, "internal20_run7.jsonl"),
        args.max_new_tokens,
        "internal20_run7",
    )
    run_eval(
        ...
        tokenizer2,
        external,
        os.path.join(args.out_dir, "external23_run7.jsonl"),
        args.max_new_tokens,
        "external23_run7",
    )
