...
mkdir -p output/qwen2.5-7b-instruct_lora_digestive_3716_9to1_dual
mkdir -p logs

echo "[INFO] Dual-GPU 7B uses GPU0+GPU1. Stop 3B training first to avoid contention."

# Swift can read distributed launch env vars and spawn DDP workers.
NPROC_PER_NODE=2 CUDA_VISIBLE_DEVICES=0,1 conda run -n qwen_sft swift sft \
  ...
  --output_dir /mnt/public/zxs/course/SFT_qwen/output/qwen2.5-7b-instruct_lora_digestive_3716_9to1_dual \
